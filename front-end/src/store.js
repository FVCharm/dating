export default {
  ...
  state: {
    ...
    user_id: window.localStorage.getItem('dating-token') ? JSON.parse(atob(window.localStorage.getItem('dating-token').split('.')[1])).user_id : 0
  },
  loginAction () {
    if (this.debug) { console.log('loginAction triggered') }
    this.state.is_authenticated = true
    this.state.user_id = JSON.parse(atob(window.localStorage.getItem('dating-token').split('.')[1])).user_id
  },
  logoutAction () {
